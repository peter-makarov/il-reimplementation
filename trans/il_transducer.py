...
from trans.defaults import COPY, DELETE, BEGIN_WORD, END_WORD, END_WORD_CHAR, \
    MAX_ACTION_SEQ_LEN

from trans.optimal_expert import Expert
from trans.actions import ConditionalCopy, ConditionalDel, ConditionalSub, \
    ConditionalIns, GenerativeEdit, ConditionalEdit
...
class Transducer:
    def __init__(self, model, vocab: il_vocabulary.Vocabularies, expert: Expert,
    ...
    def remap_actions(action_scores: Dict[Any, float]) -> Dict[Any, float]:
        ...
        for action, score in action_scores.items():
            if action == END_WORD:
                remapped_action = END_WORD_CHAR     # TODO: These should be objects as well
            elif isinstance(action, ConditionalEdit):
                remapped_action = action
            elif isinstance(action, GenerativeEdit):
    ...
    def transduce(self, input_: str, encoded_input: List[int],
        ...
        while len(action_history) <= MAX_ACTION_SEQ_LEN:
            ...
            if isinstance(action, ConditionalCopy):
                ...
                alignment += 1
                output.append(action.new)
            elif action == END_WORD_CHAR:
                break
            else:
                raise ValueError(f"Unknown action: {action}.")

        return Output(action_history, "".join(output), losses)
