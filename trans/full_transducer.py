...
from trans.defaults import COPY, DELETE, BEGIN_WORD, END_WORD, UNK, \
    MAX_ACTION_SEQ_LEN
...
def log_sum_softmax_margin_loss(indices, logits, logits_len, costs=None,
                                valid_actions=None, verbose=False):
    ...
    if valid_actions is not None:
        try:
            ...
            print("np.ones_like(logits), valid_actions, costs: ",
                  np.ones_like(logits), valid_actions, costs)
            raise e
    if costs is not None:
...
def log_sum_softmax_loss(indices, logits, logits_len, valid_actions=None,
                         verbose=False):
...
def cost_actions(actions, del_cost=1, ins_cost=1, copy_cost=0):
    return sum(
        0 if a == COPY else count * del_cost if a == DELETE else count * ins_cost
        for a, count in Counter(actions).items())


def edit_cost_matrix(source, target, del_cost=1, ins_cost=1, sub_cost=2,
                     copy_cost=0):
...
def oracle_with_rollout(word, target_word, rest_of_input, valid_actions,
        ...
        if errors:
            ...
            if verbose:
                print(u'Word contains at least {} errors: {}, {}, {}'.format(
                    num_errors,
                    ''.join(word[:-1]) + '(' + word[-1] + ')',
                    ''.join([c for i, c in enumerate(word[:-1]) if
                             i not in errors]) + '(' + word[-1] + ')',
                    action2string(target_word, vocab)), errors)
            len_word -= num_errors
        try:
            if len_word and (len_word > len_target_word or  # overgenerated !
                             word[-1] != vocab.char.i2w[target_word[
                        len_word - 1]]):  # generated a wrong char
                if verbose:
                    if len_word > len_target_word:
                        message = ''.join(word), action2string(target_word,
                                                               vocab)
                    else:
                        message = word[-1], vocab.char.i2w[
            ...
            print('len_word, word, target word: ', len_word, ''.join(word),
                  action2string(target_word, vocab))
            raise e
    ...
    if len_word >= len_target_word:
        ...
        target_char_i = target_word[
            len_word]  # next target char, unicode => Works because of param_tying!!!
        ...
        if DELETE in valid_actions:
            ...
            if rollout:
                ...
                cost = del_cost + \
                       edit_cost_matrix(rest_of_input[1:],  # delete one symbol
        ...
        if COPY in valid_actions and target_char_i == top_of_buffer:
            ...
            if rollout:
                ...
                cost = copy_cost + \
                       edit_cost_matrix(rest_of_input[1:],  # delete one symbol
                                        target_word[len_word + 1:])[
                           -1, -1]  # insert this symbol
                if verbose == 2:
                    print(u'COPY COST (ref.): {}, {}, {}'.format(
                        action2string(rest_of_input[1:], vocab),
        ...
        if target_char in vocab.act.w2i:  # if such an action exists ...
            ...
            if rollout:
                ...
                _, prediction, predicted_actions = rollout(
                    COPY if bias_inserts_on else insert_target_char)
                cost = cost_actions(predicted_actions)
                if verbose == 2:
                ...
                if bias_inserts_on:  # ENCOURAGE WITH ORACLE INSERTS
                    cost = copy_cost + edit_cost_matrix(rest_of_input[1:],
                                                        # delete one symbol
                                                        target_word[
                                                        len_word + 1:])[
                        -1, -1]  # insert this symbol
                    ...
                    cost = ins_cost + edit_cost_matrix(rest_of_input,
                                                       target_word[
                                                       len_word + 1:])[
                        -1, -1]  # insert one symbol
                if verbose == 2:
                    print(u'INSERT COST (ref.): {}, {}, {}'.format(
                        action2string(rest_of_input, vocab),
                        action2string(target_word[len_word + 1:], vocab), cost))
            action_costs.append(cost)

        if verbose == 2:
            print('Target char:', target_char_i, target_char)
            print('Actions, action costs:', action2string(actions, vocab),
                  action_costs)
            print('Top of the buffer:', top_of_buffer,
                  action2string([top_of_buffer], vocab))
    ...
    if verbose == 2:
        ...
        print('Valid actions:', valid_actions,
              action2string(valid_actions, vocab))
        print('Optimal actions:', optimal_actions,
              action2string(optimal_actions, vocab))
        print('Costs:', costs)
...
class Transducer(object):
    def __init__(self, model, vocab, char_dim=100, action_dim=100, feat_dim=20,
        ...
        self.INSERTS = list(
            range(self.vocab.number_specials, self.NUM_ACTS)) + [UNK]
    ...
    def _features(self, model):
        ...
        self.CHAR_LOOKUP = model.add_lookup_parameters(
            (self.NUM_CHARS, self.CHAR_DIM))
        if self.param_tying:
            self.ACT_LOOKUP = self.CHAR_LOOKUP
            print(
                'NB! Using parameter tying: Chars and actions share embedding matrix.')
        else:
            self.ACT_LOOKUP = model.add_lookup_parameters(
                (self.NUM_ACTS, self.ACTION_DIM))
        ...
        if not self.FEAT_DIM:
            ...
            self.FEAT_LOOKUP = model.add_lookup_parameters(
                (self.NUM_FEATS, self.FEAT_DIM))
            if self.pos_emb:
                self.POS_LOOKUP = model.add_lookup_parameters(
                    (self.NUM_POS, self.FEAT_DIM))
                ...
                print(
                    'All feature-value pairs are taken to be atomic, except for POS.')
            else:
                ...
                if self.avm_feat_format:
                    ...
                    self.FEAT_INPUT_DIM = (
                                                      self.NUM_FEATS - 1) * self.FEAT_DIM  # -1 for UNK
            ...
            if self.COMPACT_FEAT_DIM:
                ...
                self.pW_feat_transform = model.add_parameters(
                    (self.COMPACT_FEAT_DIM, self.FEAT_INPUT_DIM))
                self.pb_feat_transform = model.add_parameters(
                    self.COMPACT_FEAT_DIM)
                self.FEAT_INPUT_DIM = self.COMPACT_FEAT_DIM

        # BiLSTM encoding lemma
        self.fbuffRNN = self.LSTM(self.ENC_LAYERS, self.CHAR_DIM,
                                  self.ENC_HIDDEN_DIM, model)
        self.bbuffRNN = self.LSTM(self.ENC_LAYERS, self.CHAR_DIM,
                                  self.ENC_HIDDEN_DIM, model)

        # LSTM representing generated word
        self.WORD_REPR_DIM = self.ENC_HIDDEN_DIM * 2 + self.ACTION_DIM + self.FEAT_INPUT_DIM
        self.wordRNN = self.LSTM(self.DEC_LAYERS, self.WORD_REPR_DIM,
                                 self.DEC_HIDDEN_DIM, model)

        self.CLASSIFIER_IMPUT_DIM = self.DEC_HIDDEN_DIM
        if self.double_feats:
            self.CLASSIFIER_IMPUT_DIM += self.FEAT_INPUT_DIM

        print(' * LEMMA biLSTM:      IN-DIM: {}, OUT-DIM: {}'.format(
            2 * self.CHAR_DIM, 2 * self.ENC_HIDDEN_DIM))
        print(' * WORD LSTM:         IN-DIM: {}, OUT-DIM: {}'.format(
            self.WORD_REPR_DIM, self.DEC_HIDDEN_DIM))
        print(' LEMMA LSTMs have {} layer(s)'.format(self.ENC_LAYERS))
        print(' WORD LSTM has {} layer(s)'.format(self.DEC_LAYERS))
        print()
        print(' * CHAR EMBEDDINGS:   IN-DIM: {}, OUT-DIM: {}'.format(
            self.NUM_CHARS, self.CHAR_DIM))
        if not self.param_tying:
            print(' * ACTION EMBEDDINGS: IN-DIM: {}, OUT-DIM: {}'.format(
                self.NUM_ACTS, self.ACTION_DIM))
        if self.FEAT_DIM:
            print(' * FEAT. EMBEDDINGS:  IN-DIM: {}, OUT-DIM: {}'.format(
                self.NUM_FEATS, self.FEAT_DIM))
        print(' * FEAT. VECTOR DIM:  {} {}'.format(self.FEAT_INPUT_DIM,
                                                   '(COMPACTED)' if self.COMPACT_FEAT_DIM else ''))

    def _classifier(self, model):
        # single-hidden-layer classifier that works on feature presentation
        # from "self._features"
        if self.MLP_DIM:
            self.pW_s2h = model.add_parameters(
                (self.MLP_DIM, self.CLASSIFIER_IMPUT_DIM))
            self.pb_s2h = model.add_parameters(self.MLP_DIM)
            feature_dim = self.MLP_DIM
            print(' * HIDDEN LAYER:      IN-DIM: {}, OUT-DIM: {}'.format(
                self.CLASSIFIER_IMPUT_DIM, feature_dim))
        else:
        ...
        print(
            ' * SOFTMAX:           IN-DIM: {}, OUT-DIM: {}'.format(feature_dim,
                                                                   self.NUM_ACTS))
    ...
    def _build_lemma(self, lemma, unk_avg, is_training):
        ...
        if is_training:
            ...
            if unk_avg:
                ...
                UNK_CHAR_EMB = dy.average(
                    [self.CHAR_LOOKUP[i] for i in range(1, self.NUM_CHARS)])
            else:
                # @TODO Pretrain it with "word dropout", otherwise
                # these are randomly initialized embeddings.
                UNK_CHAR_EMB = self.CHAR_LOOKUP[UNK]
            lemma_enc = [
                self.CHAR_LOOKUP[c] if c < self.NUM_CHARS else UNK_CHAR_EMB for
                c in lemma]
        return lemma_enc
    ...
    def _build_features(self, pos, feats):
        ...
        if self.FEAT_DIM:
            ...
            if self.COMPACT_FEAT_DIM:
                ...
                feats_enc = self.COMPACT_NONLIN(
                    W_feat_transform * feats_enc + b_feat_transform)
        else:
    ...
    def l2_norm(self, with_embeddings=True):
        ...
        for rnn in (self.fbuffRNN, self.bbuffRNN, self.wordRNN):
            for exp in (e for layer in rnn.get_parameter_expressions() for e in
                        layer):
                if len(exp.dim()[0]) != 1:
    ...
    def transduce(self, lemma, feats, oracle_actions=None, external_cg=True,
        ...
        while len(action_history) <= MAX_ACTION_SEQ_LEN:
            ...
            if oracle_actions is None:
                ...
                if rollout_on or (
                        not dynamic['global_rollout'] and np.random.rand() >
                        dynamic['rollout_mixin_beta']):
                    # the second disjunct allows for model roll-out applied locally
                    rollout = lambda action: self.rollout(action,
                                                          dy.log_softmax(logits,
                                                                         valid_actions),
                                                          action_history,
                                                          features, decoder,
                                                          encoder, word,
                ...
                if dynamic['loss'] == 'softmax-margin':
                    ...
                    loss = cost_sensitive_reg_loss(optim_actions, logits,
                                                   self.NUM_ACTS,
    ...
    def rollout(self, action, log_probs, action_history, features, decoder,
                encoder, word, W_act, b_act):
        ...
        if action != END_WORD:
            ...
            if action == COPY:
                ...
                # one of the INSERT actions
                assert action in self.INSERTS, (
                action, self.vocab.act.i2w[action])
                # 1. Append inserted character to the output word
                char_ = self.vocab.act.i2w[action]
                word.append(char_)
